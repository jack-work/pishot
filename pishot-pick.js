...
if (!message) {
	console.log("Usage: pishot-pick.js 'message'");
	console.log("\nExample:");
	console.log('  pishot-pick.js "Click the submit button"');
	process.exit(1);
}
	...
	if (!window.pick) {
		window.pick = async (message) => {
			...
			return new Promise((resolve) => {
				...
				const updateBanner = () => {
					banner.textContent = `${message} (${selections.length} selected, Ctrl+click to add, Enter to finish, ESC to cancel)`;
				};
				updateBanner();
